...
import { Plus, Edit, Trash2, Loader2, Clock, Send, RefreshCw } from "lucide-react";
import {
...
const Reminders = () => {
  ...
  const fetchData = async () => {
    ...
    // Enriquecer com nome do template recorrente
    const enriched = remindersData.map((r: any) => {
      const recurrentTemplate = r.recurrent_template_id
        ? templatesData.find((t: any) => t.id === r.recurrent_template_id)
        : null;
      return { ...r, recurrent_template: recurrentTemplate || null };
    });
  ...
  useEffect(() => {
    if (editing) {
      setForm({
        ...
        recurrent_template_id: editing.recurrent_template_id || "",
        days_offset: String(editing.days_offset),
        is_active: editing.is_active,
        send_time: editing.send_time || "08:00",
      });
    } else {
      setForm({ name: "", template_id: "", recurrent_template_id: "", days_offset: "-3", is_active: true, send_time: "08:00" });
    }
  }, [editing, modalOpen]);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      recurrent_template_id: form.recurrent_template_id || null,
  ...
  const handleSendNow = async (reminder: any) => {
    if (!reminder.template_id && !reminder.recurrent_template_id) {
    ...
    try {
      ...
      toast({ title: "Envio concluído!", description: data?.message || "Mensagens enfileiradas com sucesso." });
    } catch (e: any) {
  ...
  const handlePreviewSend = async (reminder: any) => {
    ...
    try {
      const now = new Date();
      const brasilia = new Date(now.getTime() + (-3 * 60 + now.getTimezoneOffset()) * 60000);
    ...
    <div className="space-y-6 animate-fade-in overflow-x-hidden">
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border/50">
              ...
              <TableHead>Template Recorrente</TableHead>
          ...
          <TableBody>
            {loading ? (
              ...
              <TableRow><TableCell colSpan={7} className="text-center py-12 text-muted-foreground">Nenhum lembrete configurado</TableCell></TableRow>
            ) : reminders.map(r => (
        ...
        {loading ? (
          ...
          <div key={r.id} className="glass-card rounded-xl p-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              ...
              <div>
                <span className="text-muted-foreground text-xs">T. Recorrente</span>
                {r.recurrent_template?.name ? (
                  <p className="truncate text-primary flex items-center gap-1">
                    <RefreshCw className="h-3 w-3" />{r.recurrent_template.name}
                  </p>
                ) : (
        ...
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-4">
            <div className="space-y-2">
              <Label>Nome *</Label>
              <Input value={form.name} onChange={e => setF("name", e.target.value)} placeholder="Ex: Aviso 3 dias antes" />
            </div>

            {/* Template padrão */}
            <div className="space-y-2">
              <Label>Template de Mensagem</Label>
              <Select value={form.template_id || "none"} onValueChange={v => setF("template_id", v === "none" ? "" : v)}>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                <Label>Template para Clientes Recorrentes</Label>
                <Badge variant="outline" className="text-xs text-primary border-primary/40 flex items-center gap-1">
                  <RefreshCw className="h-2.5 w-2.5" />
                  Recorrente
                </Badge>
              </div>
              <Select value={form.recurrent_template_id || "none"} onValueChange={v => setF("recurrent_template_id", v === "none" ? "" : v)}>
                <SelectTrigger><SelectValue placeholder="Selecionar template recorrente" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Nenhum (pula clientes recorrentes)</SelectItem>
                  {templates.map(t => (
                    <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>
                  ))}
                </SelectContent>
              ...
              <p className="text-xs text-muted-foreground">
                Aplicado apenas em lembrete pré/no-vencimento. Se vazio, clientes recorrentes não recebem mensagem neste lembrete.
                Após o vencimento, recebem o template padrão (sinal de cartão recusado).
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Input type="number" value={form.days_offset} onChange={e => setF("days_offset", e.target.value)} />
                <p className="text-xs text-muted-foreground">Negativo = antes. 0 = no dia. Positivo = depois.</p>
              </div>
              <div className="space-y-2">
                <Label>Horário de envio</Label>
                <Input type="time" value={form.send_time} onChange={e => setF("send_time", e.target.value)} />
              </div>
            </div>
      ...
      <AlertDialog open={!!confirmSend} onOpenChange={() => setConfirmSend(null)}>
        <AlertDialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-lg">
              ...
              <div className="space-y-3 pt-2">
                {confirmSend && (
                  <div className="rounded-lg border border-border p-3 space-y-2 text-sm">
                    ...
                    {confirmSend.recurrent_template?.name && (
                      <div className="flex justify-between gap-2">
                        <span className="text-muted-foreground shrink-0 flex items-center gap-1">
                          <RefreshCw className="h-3 w-3" /> Recorrente:
                        </span>
                        <span className="font-medium text-primary text-right truncate">{confirmSend.recurrent_template.name}</span>
                      </div>
                    )}
                    <div className="flex justify-between gap-2">
          ...
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmSend} disabled={loadingPreview || previewCount === 0}>
