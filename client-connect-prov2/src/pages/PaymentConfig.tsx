  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      const { data } = await supabase
        ...
        .eq("user_id", user.id)
        .single();
      if (data?.mercadopago_access_token) setToken(data.mercadopago_access_token);
      if (data?.mercadopago_public_key) setPublicKey(data.mercadopago_public_key);
      setLoading(false);
    };
    fetchProfile();
  }, [user]);
  ...
  const handleSave = async () => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        mercadopago_public_key: publicKey.trim() || null,
      })
      .eq("user_id", user.id);
    if (error) {
    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="glass-card rounded-xl p-6 max-w-xl space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Credenciais → Access Token de produção
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Credenciais → Public Key de produção (necessária para pagamento com cartão inline)
          </p>
        </div>
      ...
      <div className="glass-card rounded-xl p-6 max-w-xl">
        ...
        <ul className="text-sm text-muted-foreground space-y-2">
          <li>1. Insira seu <strong>Access Token</strong> e <strong>Public Key</strong> de produção do Mercado Pago</li>
          <li>2. Teste a conexão para garantir que está funcionando</li>
          <li>3. Seus clientes poderão pagar via <strong>PIX</strong> (QR Code) ou <strong>Cartão de crédito</strong> (direto na fatura)</li>
          <li>4. Pagamentos no cartão têm <strong>acréscimo de 10%</strong> (configurável)</li>
          <li>5. Ao confirmar o pagamento, o vencimento do cliente é <strong>renovado automaticamente</strong></li>
        </ul>
      </div>
    </div>
  );
