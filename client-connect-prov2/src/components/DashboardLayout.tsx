...
export function DashboardLayout() {
  ...
  useEffect(() => {
    ...
    const checkSubscription = async () => {
      // Safety timeout - if check takes more than 5s, force logout
      timeout = setTimeout(async () => {
        console.warn("DashboardLayout: checkSubscription timeout, forcing logout");
        await supabase.auth.signOut();
        setCheckingAccess(false);
      }, 5000);

      try {
        // First verify the session is actually valid
