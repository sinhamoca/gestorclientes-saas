...
import {
  ...
  Timer, Package, ShieldCheck, ShieldOff, Smartphone, X,
} from "lucide-react";
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      const { data } = await supabase
        .from("profiles")
        .select("wuzapi_url, wuzapi_token, pix_key, msg_delay_seconds, msg_batch_size, msg_batch_pause_seconds, totp_enabled")
        .eq("user_id", user.id)
        .single();
      if (data) {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="glass-card rounded-xl p-6 max-w-xl">
        ...
        {/* ── Estado: Desativando (pede código) ── */}
        {totpDisableMode && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 p-3 rounded-lg bg-destructive/5 border border-destructive/20">
              <ShieldOff className="h-5 w-5 text-destructive shrink-0" />
              <p className="text-sm text-muted-foreground">
                Para desativar, digite o código atual do seu autenticador.
              </p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <Button variant="ghost" onClick={() => { setTotpDisableMode(false); setTotpDisableCode(""); }}>
                Cancelar
              </Button>
            </div>
          </div>
      ...
      {configured ? (
        ...
        <div className="glass-card rounded-xl p-6 max-w-xl">
          ...
          <p className="text-sm text-muted-foreground">
            O administrador do sistema precisa configurar as credenciais WhatsAp para sua conta.
      ...
      <div className="glass-card rounded-xl p-6 max-w-xl">
        ...
        <ul className="text-sm text-muted-foreground space-y-2">
          <li>1. O administrador configura as credenciais WhatsAp para sua conta</li>
