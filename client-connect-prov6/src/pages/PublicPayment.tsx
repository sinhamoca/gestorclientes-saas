...
interface ClientPayment {
  ...
  invoice_methods?: InvoiceMethods;
}
...
const PublicPayment = () => {
  ...
  // Invoice methods (controlado pelo dono do cliente)
  const [invoiceMethods, setInvoiceMethods] = useState<InvoiceMethods>({ pix: true, pix_page: true, card: true });
  ...
  // ─── Fetch promoções ativas (silencioso) ───
  useEffect(() => {
    if (!token) return;
    const fetchPromotions = async () => {
      try {
        const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/public-promotions?token=${token}`;
        const res = await fetch(url, {
          headers: { apikey: import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY },
        });
        if (res.ok) {
      ...
      } catch { /* silencioso — não bloqueia a fatura */ }
    };
    fetchPromotions();
  }, [token]);

  // ─── Verifica se jogo está ativo (silencioso) ───
  useEffect(() => {
    if (!token) return;
    const checkGame = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          setGameEnabled(data.isEnabled === true);
        }
      } catch { /* silencioso */ }
    };
    checkGame();
      ...
      <div className="glass-card rounded-2xl p-6 sm:p-8 max-w-md w-full space-y-6">
        ...
        {!client.is_active ? (
          ...
          <div className="space-y-3">
            ...
            {/* Separador (só aparece se tem PIX e cartão) */}
        ...
        {/* ── Botão Jogo — só aparece se o jogo estiver ativo ── */}
        {gameEnabled && (
          <a href={`/jogar/${token}`} className="block w-full">
            <Button
              variant="ghost"
              className="w-full text-primary hover:text-primary hover:bg-primary/10 border border-primary/20"
              size="sm"
              type="button"
            >
        ...
        {/* ── Botão Promoções — só aparece se existir pelo menos uma promoção ativa ── */}
        {hasPromotions && (
          <Button
            variant="ghost"
            className="w-full text-purple-500 hover:text-purple-600 hover:bg-purple-500/10 border border-purple-500/20"
            size="sm"
        ...
        <DialogContent className="max-w-[calc(100vw-2rem)] sm:max-w-md max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            {promotions.map(promo => (
              <div key={promo.id} className="rounded-xl border border-border/50 overflow-hidden">
                {/* Cabeçalho */}
                <div className="p-4 space-y-1">
                  <div className="flex items-center justify-between gap-2">
                    <span className="font-semibold text-foreground">{promo.name}</span>
                    {promo.drawn_at ? (
                      <Badge className="bg-purple-500/15 text-purple-500 border-purple-500/30 text-xs flex items-center gap-1 shrink-0">
                  ...
                  <p className="text-sm text-muted-foreground">
                    <span className="font-medium text-foreground">{promo.prize_count}</span> prêmio(s) de{" "}
                    <span className="font-medium text-primary">{promo.discount_percent}% de desconto</span>
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {promo.start_date.split("-").reverse().join("/")}
                    {promo.end_date ? ` → ${promo.end_date.split("-").reverse().join("/")}` : " → sem prazo"}
                  </p>
                  {promo.drawn_at && (
                    <p className="text-xs text-purple-500">
                      Sorteado em {new Date(promo.drawn_at).toLocaleDateString("pt-BR")}
                    </p>
                  )}
                </div>

                {/* Ganhadores (pós-sorteio) */}
                {promo.drawn_at && promo.winners.length > 0 && (
                  <div className="border-t border-border/30 bg-muted/20 px-4 py-3 space-y-2">
                    ...
                    <div className="space-y-1.5">
                      {promo.winners.map((w, i) => (
                        <div key={i} className="flex items-center justify-between px-2 py-1.5 rounded-lg bg-background border border-border/30">
                          <div className="flex items-center gap-2">
                ...
                {/* Aguardando sorteio */}
                {!promo.drawn_at && (
                  <div className="border-t border-border/30 bg-primary/5 px-4 py-3">
                    <p className="text-xs text-muted-foreground text-center">
                      🎉 Pague em dia para participar do sorteio!
                    </p>
                  </div>
                )}
              </div>
