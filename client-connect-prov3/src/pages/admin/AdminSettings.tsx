...
export default function AdminSettings() {
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      // Fetch settings for THIS admin (user_id = user.id)
  ...
  const handleSaveRenewal = async () => {
    ...
    const { error: e1 } = await upsertSetting("renewal_api_url", renewalUrl.trim());
    const { error: e2 } = await upsertSetting("renewal_api_key", renewalKey.trim());
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="glass-card rounded-xl p-6 max-w-xl">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Configure a URL e a chave da API externa de renovação. Seus revendedores usarão estas credenciais.
        </p>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label>URL da API</Label>
            <Input value={renewalUrl} onChange={(e) => setRenewalUrl(e.target.value)} placeholder="http://178.156.241.26:4000/api/v1" />
          </div>
          <div className="space-y-2">
            <Label>API Key</Label>
